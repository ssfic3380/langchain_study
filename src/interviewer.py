...
class Interviewer:
    ...
    def analyze_cover_letter(self):
        input = f"""
        {self.company_name}의 {self.job_name} 포지션을 뽑는 면접관의 입장에서, 신입 지원자의 자기소개서를 평가해봅시다.
        좋았던 부분(좋은 점)과 개선이 필요한 부분(아쉬운 점)을 구분하여 평가하세요. 개선이 필요한 부분을 이야기 할 때에는 예시를 들어주세요. 이 외의 답변은 하지 마세요.
    ...
    def generate_interview_questions(self):
        input = f"""
        당신은 {self.company_name} 회사의 면접관으로서, {self.job_name} 포지션에 지원한 신입 지원자를 대상으로 면접 질문을 준비하고 있습니다.
        지원자의 자기소개서와 회사의 모집 공고를 바탕으로, 지원자의 적합성을 판단할 수 있는 최소 4개 이상의 면접 질문을 만들어보세요.
        이 질문들은 지원자의 기술, 경험, 그리고 문화 적합성에 대해 탐구하는 데 도움이 되어야 합니다.
        
        단, 자기소개서에서 나왔던 질문과 비슷한 내용의 질문을 다시 하면 안됩니다. 모집 공고를 직접적으로 언급해서도 안됩니다.
        그리고 이 지원자는 아직 {self.company_name} 회사에서 일해본 적이 없는 신입 지원자임을 기억하세요.
    ...
    def analyze_interview_answer(self, question, answer):
        input = f"""
        당신은 {self.company_name}라는 회사의 면접관으로서, {self.job_name} 포지션에 지원한 신입 지원자의 답변을 평가하고 있습니다. 아래 제공된 질문과 답변을 바탕으로 분석하십시오. 
    ...
    def generate_follow_up_question(self, question, answer):
        input = f"""
        당신은 {self.company_name}라는 회사의 면접관으로서, {self.job_name} 포지션에 지원한 신입 지원자의 답변을 보고 후속 질문을 만들고 있습니다.
        아래 제공된 질문과 답변을 바탕으로 후속 질문 1개를 준비하되, 후속 질문이 필요 없다면 그런 사실을 명시하십시오.
    ...
    def analyze_interview_answer_and_generate_follow_up_question(self, question, answer):
        input = f"""
        당신은 {self.company_name}라는 회사의 면접관으로서, {self.job_name} 포지션에 지원한 신입 지원자의 답변을 분석하고 평가하고 있습니다. 아래 제공된 질문과 답변을 바탕으로 분석하십시오. 
    ...
    def analyze_interview_answer_and_generate_follow_up_question_with_guideline(self, question, answer):
        ...
        그리고 분석한 결과를 바탕으로 후속 질문 1개를 준비하십시오. 질문 분류가 '지식'인 경우, 지원자가 핵심적인 내용을 충분히 이해하고 있는지 확인하는 후속 질문을 준비해야 합니다. 만약 지원자가 답변에서 중요한 내용을 빠트렸다면, 그 부분에 대해 물어봅니다.
